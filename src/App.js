...
class App extends Component {
  ...
  checkMatch = thumbnaiUrl => {
    const {activeImage} = this.state

    if (thumbnaiUrl === activeImage) {
      const No = Math.floor(Math.random() * imagesList.length)
      this.setState(prevState => ({
        score: prevState.score + 1,
        activeImage: imagesList[No].imageUrl,
      }))
    } else {
      clearInterval(this.timerId)
      this.setState({gameActive: true})
  ...
  render() {
    const {score, timer, gameActive, activeImage, activeTab} = this.state
    ...
    return (
      <div>
        <ul className="header-bg">
          ...
          <div className="score-timer-card">
            <li className="list-item">
            ...
            <div className="timer-card">
              ...
              />
              <li className="list-item">
                <p className="header-text">{timer} sec</p>
              </li>
            </div>
          </div>
        </ul>
        <div className="game-bg">
          {gameActive ? (
            this.renderGameFailure()
          ) : (
            <div className="game-background">
              <img src={mainImage} alt="match" className="active-img" />
              ...
              <ul className="thumbnail-card">
                {activeImagesList.map(eachImage => (
                  <li key={eachImage.id}>
                    <button
                      type="button"
                      className="thumbnail-img"
                      value={eachImage.imageUrl}
                      onClick={() => this.checkMatch(eachImage.imageUrl)}
                    >
                      <img
